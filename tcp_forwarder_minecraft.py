...
# We modified it to transform tcp into Minecraft video packets (tcp).
import socket
import select
import time
import sys
import encode_as_minecraft
...
class TheServer:
    ...
    def on_recv(self):
        ...
        if self.channel[self.s] == self.forward:
            output = encode_as_minecraft.transform_string(data)
            for i in range(0, len(output)):
                self.channel[self.s].send(output[i].decode('hex'))
        else:
            self.channel[self.s].send('hello world')            

if __name__ == '__main__':
        server = TheServer('', 10000)
        try:
            server.main_loop()
