...
class TheServer:
    ...
    def on_accept(self):
		### It seems that UDP doesn't need connect
        ...
        if forward:
            ...
            self.addr[clientsock] = forward_to
            self.addr[forward] = clientaddr
        else:
    ...
    def on_recv(self):
        ...
        if self.addr[self.s] == forward_to:
            print 'udp'
            # Transform the packet into NTP
            output = encode_as_ntp.transform_string(data)
            print output
            for i in range(0, len(output)):
                self.channel[self.s].sendto(output[i].decode('hex'), self.addr[self.s])
        else:
            print 'tcp'
